...
* @changes 30-11-2009 Cut last comma from js scriptData array in order to make IE happy - thanks Jerry2801
* 
*/
        ...
        if (($n)>0 && array_keys($value)!==range(0,$n-1)) {
            foreach($value as $k=>$v) 
            {
                ...
                elseif($k == 'scriptData')
                {
                    ...
                    $sd = substr($sd, 0, -1);   //  cut last comma from js array to make IE happy - jerry2801 30-11-2009
                    $es[] = "'" . $k . "':{" . $sd . "}";              
                }  else
                    $es[] = "'" . $k . "':'" .$v . "'";
